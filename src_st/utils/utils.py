...
def viz_structure(upload: Any, is_native: bool = True, session_number: int = 77):
    prefix = os.path.join("data", "user", f"tmp_{session_number}")
    bytes_data = upload.read()
    c_dir = "native" if is_native else "preds"
    prefix = os.path.join(prefix, c_dir)
    out_path = os.path.join(prefix, upload.name)
    save_to_file(bytes_data, out_path)
    return out_path
